...
=para/=. This draws on the =para= portion of my handle.
...
(define-advice which-func-ff-hook (:after () header-line)
  "Route which-func into the header line instead of the mode line."
...
(use-package ob-mermaid :ensure nil
  :config (setopt ob-mermaid-cli-path "mmdc"))
...
(defvar para/org-agenda-dir        (concat org-directory "/agendas")                 "Top level org directory for Getting Things Done (GTD) organizer files.")
(defvar para/org-calendar          (concat para/org-agenda-dir "/calendar.org")      "Calendar for scheduled items.")
(defvar para/org-habits            (concat para/org-agenda-dir "/habits.org")        "Habits for periodic TODOs.")
(defvar para/org-ideas             (concat para/org-agenda-dir "/ideas.org")         "Idea capture and ideation.")
(defvar para/org-inbox             (concat para/org-agenda-dir "/inbox.org")         "The collection bin for everything to be refiled.")
(defvar para/org-incubate-dir      (concat para/org-agenda-dir "/incubate")          "Categories of in-actionable things to incubate.")
(defvar para/org-emacs-maybe       (concat para/org-incubate-dir "/emacs-maybe.org") "Someday agenda for Emacs related things.")
(defvar para/org-maybe-projects    (concat para/org-incubate-dir "/projects.org")    "Someday agenda for project ideas.")
(defvar para/org-someday           (concat para/org-incubate-dir "/someday.org")     "Someday agenda for things I may want to revisit.")
(defvar para/org-travel            (concat para/org-incubate-dir "/travel.org")      "Someday agenda for travel related things.")
(defvar para/org-learning          (concat para/org-agenda-dir "/learning.org")      "Agenda for structured learning.")
(defvar para/org-organizer         (concat para/org-agenda-dir "/organizer.org")     "Core organizer tracking prioritized actionable work.")
(defvar para/org-people-dir        (concat para/org-agenda-dir "/people")            "Org files specific to people.")
(defvar para/org-projects          (concat para/org-agenda-dir "/projects.org")      "Org projects index and tasks.")
(defvar para/org-manager           (concat para/org-people-dir "/manager.org")       "Actionable captures for 1:1s with my manager.")
(defvar para/org-retrospective     (concat para/org-agenda-dir "/retro.org")         "Capture target for retro related info or actionable items.")
(defvar para/org-review            (concat para/org-agenda-dir "/review.org")        "Reference on and journal for reflection.")
(defvar para/org-tickler           (concat para/org-agenda-dir "/tickler.org")       "Time relevant reminders for \\='decide to do later\\=' items.")
(defvar para/org-waiting           (concat para/org-agenda-dir "/waiting.org")       "Delegated or blocked items awaiting external action.")
(defvar para/org-blog              (concat org-directory "/blog")                    "Top level org directory for blog posts.")
(defvar para/org-brain-dir         (concat org-directory "/brain")                   "Top level org directory for \\='org-brain\\=' reference material.")
(defvar para/org-checklist-dir     (concat org-directory "/checklists")              "Top level org directory for action oriented reference; do/certify.")
(defvar para/org-commonplace-dir   (concat org-directory "/commonplace")             "Top level org directory for \\='commonplace book\\=' material.")
(defvar para/org-journal-dir       (concat org-directory "/journal")                 "Top level org directory for journal entries.")
(defvar para/org-ledger-dir        (concat org-directory "/ledger")                  "Top level org directory for accounting/budgeting ledgers.")
(defvar para/org-projects-dir      (concat org-directory "/projects")                "Top level org directory for projects.")
(defvar para/org-templates-dir     (concat org-directory "/templates")               "Top level org directory for templates.")
(defvar para/org-cap-templates-dir (concat para/org-templates-dir "/org-capture")    "Directory for \\='org-capture\\=' template files.")
(defvar para/org-incidents-dir     (concat org-directory "/incidents")               "Directory for incident response capture files.")

(defvar org-default-notes-file para/org-inbox)
#+end_src
...
What I want is to have the same hierarchy in the org file and it's archive file. Then when archiving a regular TODO, it
moves to the same place in the arhcive file.
...
** TODO Org Reporting with =org-ql=
...
*** Agenda/Capture Builders
Small helpers that expand data alists into =org-agenda-custom-commands= and =org-capture-templates= entries,
reducing repeated =org-ql-block= and capture boilerplate while keeping literate section boundaries.

#+begin_src emacs-lisp
;;; BEGIN Agenda/Capture Builders

(defun para/org-ql-view-block (header query &rest extra-settings)
  "Return an org-ql-block form for QUERY with HEADER."
  `(org-ql-block ',query
                 ((org-ql-block-header ,header)
                  ,@extra-settings)))

(defvar para/org-agenda-custom-commands nil)

(defun para/org-agenda-extend (commands)
  "Append top-level agenda commands/groups to para/org-agenda-custom-commands."
  (setq para/org-agenda-custom-commands
        (append para/org-agenda-custom-commands commands)))

(defun para/org-ql-blocks-from-alist (specs)
  "SPECS is ((HEADER QUERY) ...).  Return a list of org-ql-block forms."
  (mapcar (lambda (spec)
            (para/org-ql-view-block (car spec) (cadr spec)))
          specs))

(defvar para/org-ql-gtd-block-specs nil
  "Alist of (SYMBOL HEADER QUERY) for reusable GTD agenda blocks.")

(defun para/org-ql-gtd-block (symbol)
  "Return a one-element list containing the GTD block form for SYMBOL."
  (let ((spec (assoc symbol para/org-ql-gtd-block-specs)))
    (list (para/org-ql-view-block (nth 1 spec) (nth 2 spec)))))

(defun para/org-ql-gtd-blocks (symbols)
  "Return a flat list of org-ql-block forms for SYMBOLS."
  (apply #'append (mapcar #'para/org-ql-gtd-block symbols)))

(defun para/org-context-commands (contexts)
  "CONTEXTS is ((KEY LABEL TAG) ...).  Return full @-group agenda entries."
  (mapcar (lambda (ctx)
            (list (concat "@" (nth 0 ctx))
                  (nth 1 ctx)
                  (list (para/org-ql-view-block
                         (format "Open %s Tasks:" (nth 1 ctx))
                         `(and (not (done)) (tags ,(nth 2 ctx)))))))
          contexts))

(defvar para/org-priority-quadrants
  '((q1 "Urgent"
        (and (todo) (not (done)) (tags "IMPORTANT" "URGENT")))
    (q2 "Important"
        (and (todo) (not (done)) (tags "IMPORTANT") (not (tags "URGENT"))))
    (q3 "Delegate"
        (and (todo) (not (done)) (tags "URGENT") (not (tags "IMPORTANT"))))
    (q4 "Decline"
        (and (todo) (not (done)) (not (habit)) (not (tags "URGENT" "IMPORTANT"))))))

(defun para/org-priority-quadrant-blocks ()
  "Return org-ql-block forms for all Eisenhower quadrants."
  (mapcar (lambda (q)
            (para/org-ql-view-block (nth 1 q) (nth 2 q)))
          para/org-priority-quadrants))

(defun para/org-priority-quadrant-command (suffix label qsym)
  "Return a single-quadrant priority-matrix agenda command."
  (let ((q (assoc qsym para/org-priority-quadrants)))
    (list (concat "p" suffix) label
          (list (para/org-ql-view-block (nth 1 q) (nth 2 q))))))

(defvar para/org-capture-templates nil)

(defun para/org-capture-extend (templates)
  "Append capture templates/groups to para/org-capture-templates."
  (setq para/org-capture-templates
        (append para/org-capture-templates templates)))

(defun para/org-capture-todo (key desc file headline priority tags
                            &optional target-type subheadings prefix)
  "Return a capture template for a TODO entry.
TARGET-TYPE is nil for file+headline or 'olp for file+olp with SUBHEADINGS.
Optional PREFIX is inserted before the %? placeholder in the entry template."
  (let* ((target (if (eq target-type 'olp)
                     `(file+olp ,file ,@subheadings)
                   `(file+headline ,file ,headline)))
         (tag-list (if (listp tags) tags (list tags)))
         (tag-str (concat ":" (mapconcat #'identity tag-list ":") ":")))
    (list key desc 'entry target
          (format "* TODO [#%s] %s%%? %s\n" priority (or prefix "") tag-str))))

(defun para/org-capture-retro-item (key label section)
  "Return a retrospective capture item template."
  (list key label 'item
        `(file+olp ,para/org-retrospective "Capture" ,section)
        "- %?"))

(defun para/org-capture-meeting (key desc section schedule)
  "Return a meeting capture entry template."
  (list key desc 'entry
        `(file+olp ,para/org-calendar "Meetings" ,section)
        (format "* MEET %%?\nSCHEDULED: %s" schedule)))

;;; END Agenda/Capture Builders
#+end_src
...
I define these by initializing the commands list with a weekly preview and appending additional groups with
=para/org-agenda-extend=. I do this so I can split the declarations up and describe them better in my literate
configuration. I'll initialize a simple weekly review that shows a span of seven days highlighting stuck projects,
open projects, and things awaiting something to happen.

#+begin_src emacs-lisp
(para/org-agenda-extend
  '(("w" "Weekly Review"
     ((agenda ""
              ((org-agenda-span 7)))
      (tags "PROJECT")
...
#+begin_src emacs-lisp
(setq para/org-ql-gtd-block-specs
      '((a-prio-open "High-Priority Unfinished Tasks"
         (and (priority "A")
              (not (or (done)
        ...
        (open-projects "Open Projects"
         (and (not (done)) (tags "PROJECT") (not (tags "HOLD"))))
        (b-prio "B Priority Tasks"
         (and (todo) (priority "B")
              (not (or (done)
                       (tags "PROJECT" "NEXT" "HOLD" "WAITING" "BLOCKED")
                       (habit)
                       (planning)))))
        (c-prio "C Priority Tasks"
         (and (todo) (priority "C")
              (not (or (done)
                       (tags "PROJECT" "NEXT" "HOLD" "WAITING" "BLOCKED")
                       (habit)
                       (planning)))))
        (week-lookahead "1-Week Look Ahead"
         (and (not (done)) (ts :from today :to 7)))
        (waiting-for "Waiting For"
         (and (not (done)) (tags "WAITING")))
        (delegated "Pending Delegate Completion"
         (todo "DELEGATED"))
        (blocked "Blocked"
         (and (not (done)) (tags "BLOCKED")))
        (on-hold "On Hold"
         (and (not (done)) (tags "HOLD")))
        (braindead "Braindead"
         (and (todo) (not (done)) (tags "braindead")))
        (for-review "For Review"
         (and (not (done)) (todo "REVIEW")
              (not (tags "HOLD" "WAITING" "BLOCKED"))))
        (completed-projects "Completed Projects"
         (and (done) (tags "PROJECT") (closed 7)))
        (a-prio-completed "A Prio Completed"
         (and (priority "A") (done) (not (tags "PROJECT")) (closed 7)))
        (b-prio-completed "B Prio Completed"
         (and (priority "B") (done) (not (tags "PROJECT")) (closed 7)))
        (c-prio-completed "C Priority Tasks"
         (and (priority "C") (done) (not (tags "PROJECT")) (closed 7)))
        (regular-prio-completed "Regular Prio Completed"
         (and (done) (not (priority "A" "B" "C")) (closed 7)))))

(para/org-agenda-extend
  `(("g" . "GTD Views")
    ("gi" "Inbox"
     ((alltodo ""
               ((org-agenda-files '(,para/org-inbox))
                (org-agenda-overriding-header "Items in my Inbox")))))
    ("gd" "Daily View"
     ,(append (para/org-ql-gtd-blocks '(a-prio-open))
              '((agenda "" ((org-agenda-span 1))))
              (para/org-ql-gtd-blocks '(next-actions open-projects b-prio c-prio))))
    ("gw" "Weekly View"
     ,(append (para/org-ql-gtd-blocks '(a-prio-open))
              '((agenda "" ((org-agenda-span 7))))
              (para/org-ql-gtd-blocks '(week-lookahead next-actions open-projects
                                                    waiting-for delegated))))
    ("gn" "Next Actions"
     ,(para/org-ql-gtd-blocks '(next-actions)))
    ("gr" "Review"
     ,(para/org-ql-gtd-blocks '(for-review)))
    ("gp" "Pending"
     ,(para/org-ql-gtd-blocks '(waiting-for delegated blocked on-hold)))
    ("gb" "Braindead"
     ,(para/org-ql-gtd-blocks '(braindead)))
    ("gc" "Recently Completed"
     ,(para/org-ql-gtd-blocks '(completed-projects a-prio-completed b-prio-completed
                                                  c-prio-completed regular-prio-completed)))))
#+end_src
...
#+begin_src emacs-lisp
(defvar para/org-contexts
  '(("a" "Administrative" "@administrative")
    ("c" "Career" "@career")
    ("m" "Communication" "@communication")
    ("f" "Finance" "@finances")
    ("g" "Guild and Community of Practice" "@guild")
    ("h" "Health and Wellness" "@health")
    ("i" "Hiring" "@hiring")
    ("H" "Home" "@home")
    ("l" "Learning Agenda" "@learn")
    ("d" "Scheduling" "@scheduling")
    ("s" "Sprint Work" "@sprint")
    ("t" "Travel and Planning" "@travel")
    ("w" "Workstation" "@workstation")))

(para/org-agenda-extend
  (cons '("@" . "Context Filters")
        (para/org-context-commands para/org-contexts)))
#+end_src
...
(para/org-agenda-extend
  `(("p" . "Priority Matrix")
    ("pq" "Quadrants" ,(para/org-priority-quadrant-blocks))
    ,(para/org-priority-quadrant-command "1" "Q1" 'q1)
    ,(para/org-priority-quadrant-command "2" "Q2" 'q2)
    ,(para/org-priority-quadrant-command "3" "Q3" 'q3)
    ,(para/org-priority-quadrant-command "4" "Q4" 'q4)))
#+end_src
...
(para/org-agenda-extend
  `(("H" . "Hygiene Filters")
    ("Hi" "Incomplete Sub-Tasks"
     (,(para/org-ql-view-block "Incomplete Sub-Tasks"
                             '(and (todo) (not (done)) (ancestors (done))))))
    ("Hd" "Projects without Open Tasks"
     (,(para/org-ql-view-block "Projects with Completed Children"
                             '(and (not (done))
                                   (tags "PROJECT")
                                   (not (children (not (done))))))))
    ("Hn" "Projects without NEXT Tasks"
     (,(para/org-ql-view-block "Projects without NEXT Action"
                             '(and (tags "PROJECT")
                                   (not (or (done)
                                            (descendants (and (not (done))
                                                              (tags "NEXT")))))))))
    ("Hp" "Missing Priority"
     (,(para/org-ql-view-block "Missing Priority"
                             '(and (todo)
                                   (not (or (done)
                                            ...
                                            (priority)))))))
    ("Ha" "Maybe Archive"
     (,(para/org-ql-view-block "Completed - Consider Archiving" '(done))))))
#+end_src
...
Lastly I set =org-agenda-custom-commands= to the value of the list I've built. I use =setq= rather than
=setopt= because =org-ql-block= query sexps do not match Org's Custom type for this variable (see startup
warning if you try =setopt=).

#+begin_src emacs-lisp
(setq org-agenda-custom-commands para/org-agenda-custom-commands)
#+end_src
...
(use-package org-journal
  :init (setopt org-journal-dir para/org-journal-dir
...
I define these by initializing my capture list with an inbox capture and appending additional templates with
=para/org-capture-extend=. I do this so I can split the configuration up and describe them better in my literate
configuration.

#+begin_src emacs-lisp
;; Org Capture Configuration
(para/org-capture-extend
  '(("i" "Inbox" entry (file+olp para/org-inbox "Capture" "Todos")
...
(defun capture-incident-response-file ()
  "Generate dated file under `para/org-incidents-dir' using an interactively provided name.
Called by \\='org-capture\\=' with no arguments, per the (file FUNCTION) target spec."
  (let ((name (read-string "Alert Name: ")))
    (expand-file-name
     (format "%s_%s.org" (format-time-string "%Y%m%d") name)
     para/org-incidents-dir)))
...
(defvar para/org-sprint-captures
  '(("sd" "Development Task" "Development" "B" ("@sprint" "TASK") nil)
    ("sv" "Review Task" "Review" "B" ("@sprint" "TASK") nil)
    ("sr" "Research Task" "Research" "B" ("@sprint" "TASK") "RESEARCH: ")
    ("ss" "Spike Task" "Research" "B" ("@sprint" "TASK") "SPIKE: ")
    ("si" "Interrupt" "Other" "A" ("@sprint" "TASK" "URGENT") nil)
    ("st" "Other Task" "Other" "B" ("@sprint" "TASK") nil)))

(para/org-capture-extend
  (cons '("s" "Sprint Capture Group")
        (mapcar (lambda (spec)
                  (para/org-capture-todo (nth 0 spec) (nth 1 spec) para/org-organizer nil
                                       (nth 3 spec) (nth 4 spec) 'olp
                                       (list "Current Sprint" (nth 2 spec))
                                       (nth 5 spec)))
                para/org-sprint-captures)))
#+end_src
...
(defvar para/org-retro-captures
  '(("rk" "Kudos" "Kudos")
    ("rg" "Goodness" "Goodness")
    ("rb" "Badness" "Badness")
    ("rz" "Kaizen" "Kaizen")))

(para/org-capture-extend
  (cons '("r" "Retrospective Capture Group")
        (mapcar (lambda (spec)
                  (apply #'para/org-capture-retro-item spec))
                para/org-retro-captures)))
#+end_src
...
(defvar para/org-office-captures
  '(("oa" "Administrative" "Administrative" "B" ("@office" "@administrative"))
    ("oc" "Communication" "Communication" "B" ("@office" "@communication"))
    ("of" "Firefighting" "Firefighting" "A" ("@office" "@firefights" "URGENT"))
    ("oh" "Hiring" "Hiring" "B" ("@office" "@hiring"))
    ("oi" "Interrupts" "Interrupts" "B" ("@office"))
    ("os" "Scheduling" "Scheduling" "B" ("@office" "@scheduling"))
    ("ot" "Task" "General" "B" ("@office"))))

(para/org-capture-extend
  (cons '("o" "Office Capture Group")
        (mapcar (lambda (spec)
                  (para/org-capture-todo (nth 0 spec) (nth 1 spec) para/org-organizer nil
                                       (nth 3 spec) (nth 4 spec) 'olp
                                       (list "Office" (nth 2 spec))))
                para/org-office-captures)))
#+end_src
...
(defvar para/org-organizer-captures
  '(("tc" "Career Task" "Career" "B" ("@career"))
    ("th" "Health Task" "Health & Wellness" "B" ("@health"))
    ("to" "Home Task" "Home" "B" ("@home"))
    ("tf" "Financial Task" "Finances" "B" ("@finances"))
    ("tg" "Guild Task" "Guild" "C" ("@guild"))
    ("tr" "Reading Task" "Reading" "B" ("@read"))
    ("tv" "Travel Task" "Travel" "B" ("@travel"))
    ("tw" "Workstation Task" "Workstation" "C" ("@workstation"))))

(para/org-capture-extend
  (cons '("t" "General Todo Capture Group")
        (append
         (mapcar (lambda (spec)
                   (para/org-capture-todo (nth 0 spec) (nth 1 spec) para/org-organizer
                                        (nth 2 spec) (nth 3 spec) (nth 4 spec)))
                 para/org-organizer-captures)
         '(("tt" "General Task" entry (file+headline para/org-organizer "Tasks")
            "\n* TODO [#C] %?\n %i\n %a\n\n")))))
#+end_src
...
reports, let's file these as someday-maybe under =para/org-incubate-dir=.
...
(defvar para/org-meeting-captures
  '(("mm" "Pop Up Meetings and One-Offs" "One Offs" "%^t")
    ("md" "Daily Meeting" "Daily" "<2020-01-01 Sat ++1d>")
    ("mw" "Weekly Meeting" "Weekly" "<2020-01-01 Sat ++1w>")
    ("m1" "1:1 Meeting" "1:1s" "<2000-01-01 Sat ++1w>")
    ("mb" "Bi-Weekly Meeting" "Bi-Weekly" "<2000-01-01 Sat ++2w>")))

(para/org-capture-extend
  (cons '("m" "Meeting Capture Group")
        (mapcar (lambda (spec)
                  (apply #'para/org-capture-meeting spec))
                para/org-meeting-captures)))
#+end_src
...
(para/org-capture-extend
  `(("j" "Journal Capture Group")
    ("jd" "Daily Goals" entry (function org-journal-find-location)
     (file ,(concat para/org-cap-templates-dir "/daily-goals.org"))
     :empty-lines 1)
...
(para/org-capture-extend
  `(("n" "Note" entry (file+olp para/org-inbox "Capture" "Notes")
     "* %?\n:PROPERTIES:\n:CREATED:%U\n:END:\n\n%i\n\nFrom: %a"
     :empty-lines 1)
    ("M" "Manager Notes" item (file+olp para/org-manager "1:1 Prep" "Capture")
     "- %?" :empty-lines 1)
    ("x" "Incident Notes" entry (file capture-incident-response-file)
     (file ,(concat para/org-cap-templates-dir "/incident.org"))
     :clock-in t)
...
(para/org-capture-extend
  '(("q" "Test Capture Group")
    ...
    ("qaa" "AA Template" entry (file+olp para/org-inbox "Test" "A" "AA")
     "* TODO [#A] %? :TASK:\n" :empty-lines 1)
    ("qaa" "AB Template" entry (file+olp para/org-inbox "Test" "A" "AB")
     "* TODO [#B] %? :TASK:\n" :empty-lines 1)
    ("qb" "Test Capture Sub-Group B")
    ("qba" "BA Template" entry (file+olp para/org-inbox "Test" "B" "BA")
     "* TODO [#A] %? :TASK:\n" :empty-lines 1)
    ("qbb" "BB Template" entry (file+olp para/org-inbox "Test" "B" "BB")
     "* TODO [#B] %? :TASK:\n" :empty-lines 1)))
#+end_src
...
#+begin_src emacs-lisp
(setopt org-capture-templates para/org-capture-templates)
#+end_src
...
(setopt org-refile-targets `((org-agenda-files :maxlevel . 6)
                             (,para/org-ideas :maxlevel . 3)
                             (,para/org-maybe-projects :maxlevel . 2)
                             (,para/org-someday :maxlevel . 4)))
...
As I use NVM to manage node versions the install directory of mermaid-cli's =mmdc= can exist in some challenging to
address spaces, especially across different machines with different operating systems.

For now I've resolved to specify an explicit path for =ob-mermaid-cli-path= in my =machine.org= local config until I
spend more time generalizing it.

Additionally, on Ubuntu there is another hurdle to overcome with AppArmor sandboxing.

Here's a sample of the config I have working on at least one machine now:

#+begin_example
(setopt ob-mermaid-cli-path "aa-exec -p chrome ~/.nvm/versions/node/v22.21.1/bin/mmdc")
#+end_example

Much appreciation to Sacha Chua for the PR and [[https://sachachua.com/blog/2025/01/org-babel-mermaid-js-and-fixing-failed-to-launch-the-browser-process-on-ubuntu-24/][blog post]] about this.
...
For TeX editing, I'm going to try using AUCTeX.

Refs:
- https://emacsdocs.org/docs/auctex/Top
...
Helpful with managing references in TeX documents.
  ...
  (defun para/ledger-save ()
    ...
    (save-excursion
      (when (buffer-modified-p)
        ...
        (save-buffer))))
  :init
  (setopt ledger-post-amount-alignment-column 80))
...
#+begin_src emacs-lisp
(defun para/reload-emacs-configuration ()
  (interactive)
...
| TODO |   | [[Org Reporting with =org-ql=][Org Reporting with =org-ql=]]                              |
