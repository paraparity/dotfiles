...
Since I don't use directory local variables for Emacs very much if at all, and considering they could load arbitrary emacs-lisp,
...
(defvar TT/org-manager          (concat TT/org-agenda-dir "/manager.org")       "Actionable captures for 1:1s with my manager.")
...
A while back I stumbled across Aaron Bieber's blog which has some really helpful posts about Emacs. Particularly of interest
here is this post: [[https://blog.aaronbieber.com/2016/09/24/an-agenda-for-life-with-org-mode.html][An Agenda for Life with org-mode]]. From it, I've lifted the following helpers.
...
let's define several GTD views for my agenda contents.
...
(setcdr (last TT/org-agenda-custom-commands)
        '(("g" . "GTD Views")
          ...
          ("gr" "Work to Review"
           ((tags "REVIEW"
                  ((org-agenda-overriding-header "Work to Review:")
                   (org-agenda-sorting-strategy '(priority-down effort-up tag-up))))))
...
**** TODO Fix Inbox Filter Error
**** TODO Add Filter for Projects without NEXT Tagged Sub-Entry
**** TODO Add Filter for TODO items without PROJECT/TASK Tags
**** TODO Add Filter for standalone TODO items: Items not under a PROJECT tag
...
interval. I don't care to get the interval exactly right, instead I'll just do a two week look-back for completed work that is
most useful on an end of sprint day.
...
*** TODO Misc Commands
A few of these commands that I'm workshopping didn't quite make sense elsewhere. Still not sure what I want to do with them yet.

#+begin_src emacs-lisp
(setcdr (last TT/org-agenda-custom-commands)
        '(("y" "Someday Maybe"
           ((todo "MAYBE"
                  ((org-agenda-files '(TT/org-someday))))))))
#+end_src
...
** TODO Configuration Reading [0/10]
