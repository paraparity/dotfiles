	...
	#+begin_src emacs-lisp
	  (setq org-tag-alist
			'(("@career"         . ?c) ; personal professional development (different than learn?)
			  ...
			  ("@live"           . ?i) ; life event planning and maintenance work
			  ...
			  ("INTERRUPT"       . ?I) ; an unexpected item that forces a context switch from focus work
			  ("PROJECT"         . ?P) ; something that takes significant effort or time, and can be decomposed into individual tasks
			  ("RESEARCH"        . ?R) ; exploratory information gathering that doesn't require standing something up
			  ("REVIEW"          . ?V) ; was originally for tracking code-review tasks (in sprint work; deprecate?)
			  ("SPIKE"           . ?S) ; exploratory work that could result in a deliverable (proof-of-concepts, etc...)
...
*** Sprint Work
	...
	I do not presently use this as an engineering manager, however I keep it around to look back on. It doesn't export
	into the final elisp configuration.

	#+begin_src emacs-lisp :tanlg no
...
*** Test Group
	Wanted to test nested groups, and this worked. Not exporting, but will keep around for learning for now. Am starting
	to get a little tired of reapeating myself with =add-to-list= pattern though. TODO?
