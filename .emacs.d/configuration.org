...
(defun TT/org-ql-view-block (header query &rest extra-settings)
  "Return an org-ql-block form for QUERY with HEADER."
  `(org-ql-block ',query
                 ((org-ql-block-header ,header)
                  ,@extra-settings)))

(defvar TT/org-agenda-custom-commands nil)

(defun TT/org-agenda-extend (commands)
  "Append top-level agenda commands/groups to TT/org-agenda-custom-commands."
  (setq TT/org-agenda-custom-commands
        (append TT/org-agenda-custom-commands commands)))

(defun TT/org-ql-blocks-from-alist (specs)
  "SPECS is ((HEADER QUERY) ...).  Return a list of org-ql-block forms."
  (mapcar (lambda (spec)
            (TT/org-ql-view-block (car spec) (cadr spec)))
          specs))

(defvar TT/org-ql-gtd-block-specs nil
  "Alist of (SYMBOL HEADER QUERY) for reusable GTD agenda blocks.")

(defun TT/org-ql-gtd-block (symbol)
  "Return a one-element list containing the GTD block form for SYMBOL."
  (let ((spec (assoc symbol TT/org-ql-gtd-block-specs)))
    (list (TT/org-ql-view-block (nth 1 spec) (nth 2 spec)))))

(defun TT/org-ql-gtd-blocks (symbols)
  "Return a flat list of org-ql-block forms for SYMBOLS."
  (apply #'append (mapcar #'TT/org-ql-gtd-block symbols)))

(defun TT/org-context-commands (contexts)
  "CONTEXTS is ((KEY LABEL TAG) ...).  Return full @-group agenda entries."
  (mapcar (lambda (ctx)
            (list (concat "@" (nth 0 ctx))
                  (nth 1 ctx)
                  (list (TT/org-ql-view-block
                         (format "Open %s Tasks:" (nth 1 ctx))
                         `(and (not (done)) (tags ,(nth 2 ctx)))))))
          contexts))

(defvar TT/org-priority-quadrants
  '((q1 "Urgent"
...
(defun TT/org-priority-quadrant-blocks ()
  "Return org-ql-block forms for all Eisenhower quadrants."
  (mapcar (lambda (q)
            (TT/org-ql-view-block (nth 1 q) (nth 2 q)))
          TT/org-priority-quadrants))

(defun TT/org-priority-quadrant-command (suffix label qsym)
  "Return a single-quadrant priority-matrix agenda command."
  (let ((q (assoc qsym TT/org-priority-quadrants)))
    (list (concat "p" suffix) label
          (list (TT/org-ql-view-block (nth 1 q) (nth 2 q))))))

(defvar TT/org-capture-templates nil)

(defun TT/org-capture-extend (templates)
  "Append capture templates/groups to TT/org-capture-templates."
  (setq TT/org-capture-templates
        (append TT/org-capture-templates templates)))

(defun TT/org-capture-todo (key desc file headline priority tags
...
(defun TT/org-capture-retro-item (key label section)
  "Return a retrospective capture item template."
  (list key label 'item
        `(file+olp ,TT/org-retrospective "Capture" ,section)
        "- %?"))

(defun TT/org-capture-meeting (key desc section schedule)
  "Return a meeting capture entry template."
  (list key desc 'entry
        `(file+olp ,TT/org-calendar "Meetings" ,section)
...
=TT/org-agenda-extend=. I do this so I can split the declarations up and describe them better in my literate
...
#+begin_src emacs-lisp
(setq TT/org-ql-gtd-block-specs
...
(TT/org-agenda-extend
  `(("g" . "GTD Views")
    ...
    ("gd" "Daily View"
     ,(append (TT/org-ql-gtd-blocks '(a-prio-open))
              '((agenda "" ((org-agenda-span 1))))
              (TT/org-ql-gtd-blocks '(next-actions open-projects b-prio c-prio))))
    ("gw" "Weekly View"
     ,(append (TT/org-ql-gtd-blocks '(a-prio-open))
              '((agenda "" ((org-agenda-span 7))))
              (TT/org-ql-gtd-blocks '(week-lookahead next-actions open-projects
                                                    waiting-for delegated))))
    ("gn" "Next Actions"
     ,(TT/org-ql-gtd-blocks '(next-actions)))
    ("gr" "Review"
     ,(TT/org-ql-gtd-blocks '(for-review)))
    ("gp" "Pending"
     ,(TT/org-ql-gtd-blocks '(waiting-for delegated blocked on-hold)))
    ("gb" "Braindead"
     ,(TT/org-ql-gtd-blocks '(braindead)))
    ("gc" "Recently Completed"
     ,(TT/org-ql-gtd-blocks '(completed-projects a-prio-completed b-prio-completed
...
(TT/org-agenda-extend
  (cons '("@" . "Context Filters")
        (TT/org-context-commands TT/org-contexts)))
#+end_src
...
(TT/org-agenda-extend
  `(("p" . "Priority Matrix")
    ("pq" "Quadrants" ,(TT/org-priority-quadrant-blocks))
    ,(TT/org-priority-quadrant-command "1" "Q1" 'q1)
    ,(TT/org-priority-quadrant-command "2" "Q2" 'q2)
    ,(TT/org-priority-quadrant-command "3" "Q3" 'q3)
    ,(TT/org-priority-quadrant-command "4" "Q4" 'q4)))
#+end_src
...
(TT/org-agenda-extend
  `(("H" . "Hygiene Filters")
    ("Hi" "Incomplete Sub-Tasks"
     (,(TT/org-ql-view-block "Incomplete Sub-Tasks"
                             '(and (todo) (not (done)) (ancestors (done))))))
    ("Hd" "Projects without Open Tasks"
     (,(TT/org-ql-view-block "Projects with Completed Children"
                             '(and (not (done))
                                   (tags "PROJECT")
                                   (not (children (not (done))))))))
    ("Hn" "Projects without NEXT Tasks"
     (,(TT/org-ql-view-block "Projects without NEXT Action"
                             '(and (tags "PROJECT")
                                   (not (or (done)
                                            (descendants (and (not (done))
                                                              (tags "NEXT")))))))))
    ("Hp" "Missing Priority"
     (,(TT/org-ql-view-block "Missing Priority"
                             '(and (todo)
                                   (not (or (done)
                                            ...
                                            (priority)))))))
    ("Ha" "Maybe Archive"
     (,(TT/org-ql-view-block "Completed - Consider Archiving" '(done))))))
#+end_src
...
=TT/org-capture-extend=. I do this so I can split the configuration up and describe them better in my literate
configuration.

#+begin_src emacs-lisp
;; Org Capture Configuration
(TT/org-capture-extend
...
(TT/org-capture-extend
  (cons '("s" "Sprint Capture Group")
        (mapcar (lambda (spec)
                  (TT/org-capture-todo (nth 0 spec) (nth 1 spec) TT/org-organizer nil
                                       (nth 3 spec) (nth 4 spec) 'olp
                                       (list "Current Sprint" (nth 2 spec))
                                       (nth 5 spec)))
                TT/org-sprint-captures)))
#+end_src
...
(TT/org-capture-extend
  (cons '("r" "Retrospective Capture Group")
        (mapcar (lambda (spec)
                  (apply #'TT/org-capture-retro-item spec))
                TT/org-retro-captures)))
#+end_src
...
(defvar TT/org-organizer-captures
...
(TT/org-capture-extend
  (cons '("t" "General Todo Capture Group")
        (append
         (mapcar (lambda (spec)
                   (TT/org-capture-todo (nth 0 spec) (nth 1 spec) TT/org-organizer
                                        (nth 2 spec) (nth 3 spec) (nth 4 spec)))
                 TT/org-organizer-captures)
...
(defvar TT/org-meeting-captures
...
(TT/org-capture-extend
  (cons '("m" "Meeting Capture Group")
        (mapcar (lambda (spec)
                  (apply #'TT/org-capture-meeting spec))
                TT/org-meeting-captures)))
#+end_src
...
#+begin_src emacs-lisp
(setopt org-capture-templates TT/org-capture-templates)
#+end_src
